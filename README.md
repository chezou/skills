...
| [`session-mailbox`](skills/session-mailbox/SKILL.md) | Runs an asynchronous review or decision loop between two AI sessions through a single append-only Markdown file, so nothing depends on messaging that reports success and silently drops. Covers the heading shape that makes new arrivals countable, cron polling that survives a restart without reading the wrong mailbox, and what to do with a mailbox once it closes. |
